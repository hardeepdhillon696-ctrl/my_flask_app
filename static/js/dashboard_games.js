...
mediaModal.addEventListener("show.bs.modal", function (event) {
    ...
    // ---------------------- Update Views ----------------------
    fetch(`/view/${videoId}`, { method: "POST" })
        .then(res => res.json())
        .then(data => {
    ...
    fetch(`/like/${videoId}`, { method: "POST" })
        ...
        .then(data => {
            if (data.likes !== undefined) {
                ...
                document.getElementById("modal-likes").textContent = 0;
            }
        }).catch(()=>{ document.getElementById("modal-likes").textContent = 0; });

    // Update the form action for liking
    const likeForm = document.getElementById("modal-like-form");
    likeForm.action = `/like/${videoId}`;

    // Handle Like button submission via fetch
...
(function () {
    ...
    class Ball {
        ...
        update() {
            ...
            for(const p of pockets){
                if(dist(this.x,this.y,p.x,p.y)<p.r){
                    this.inPocket=true; this.vx=this.vy=0; onPotted(this); break;
                }
            }
        }
        draw(ctx){
    ...
    function rackBalls(){
        ...
        const bag=Array(7).fill("red").concat(Array(7).fill("yellow")); bag.pop();
    ...
    function onPotted(ball) {
        ...
        if (ball.isBlack) { blackPottedThisShot = true; ball.inPocket = true; return; }
    ...
    function endOfShot(){
        if (blackPottedThisShot) {
            ...
            if (myColor && playerClearedAll(myColor)) {
                endGame(getPlayerName(currentPlayer), getPlayerName(otherPlayer()), false);
            } else {
                endGame(getPlayerName(otherPlayer()), getPlayerName(currentPlayer), true);
            }
            ballsMoving = false; updateBallCount(); return;
        }

        const myColor=ownership[currentPlayer];
        if(myColor && firstContact && firstContact.color!==myColor && !firstContact.isBlack){
            announce(`❌ Foul: ${getPlayerName(currentPlayer)} hit wrong ball first!`);
            switchTurn(); resetCueToKitchen(); return;
        }
        if(!firstContact && !scratchThisShot){
            announce(`❌ Foul: ${getPlayerName(currentPlayer)} missed all balls!`);
            switchTurn(); resetCueToKitchen(); return;
        }
        if(myColor && pottedThisShot.length>0){
            const onlyOpponentBalls=pottedThisShot.every(b=>b.color!==myColor && !b.isBlack);
            if(onlyOpponentBalls){ announce(`❌ Foul: ${getPlayerName(currentPlayer)} potted only opponent's balls!`); switchTurn(); resetCueToKitchen(); return; }
        }
        if(scratchThisShot){ resetCueToKitchen(); announce(`❌ Scratch! Ball in hand for ${getPlayerName(otherPlayer())}.`); switchTurn(); return; }
    ...
    function resetCueToKitchen(){ cueBall.inPocket=false; cueBall.x=TABLE.rail+120; cueBall.y=TABLE.height/2; cueBall.vx=cueBall.vy=0; }
    ...
    canvas.addEventListener("pointerup",(e)=>{ if(!aiming) return; aiming=false; canvas.releasePointerCapture(e.pointerId); const dx=aimStart.x-aimCurrent.x, dy=aimStart.y-aimCurrent.y, L=Math.hypot(dx,dy); if(L<2) return; const speed=clamp(L*0.6,0,60); cueBall.vx=(dx/L)*speed; cueBall.vy=(dy/L)*speed; ballsMoving=true; pottedThisShot=[]; scratchThisShot=false; blackPottedThisShot=false; firstContact=null; });

    // ---------------------- Drawing ----------------------
    function drawTable(){
        const clothGradient=ctx.createLinearGradient(0,0,0,TABLE.height);
        clothGradient.addColorStop(0,"#2e5d3f"); clothGradient.addColorStop(1,"#1e3d2b"); ctx.fillStyle=clothGradient; ctx.fillRect(0,0,TABLE.width,TABLE.height);
    ...
    function drawCue(){ if(ballsMoving||gameOver) return; if(!aiming) return; const dx=aimCurrent.x-cueBall.x, dy=aimCurrent.y-cueBall.y; stickAngle=Math.atan2(dy,dx); ctx.save(); ctx.translate(cueBall.x,cueBall.y); ctx.rotate(stickAngle);
        const grad=ctx.createLinearGradient(-stickLength,0,0,0); grad.addColorStop(0,"#3e2723"); grad.addColorStop(0.5,"#795548"); grad.addColorStop(1,"#d7ccc8"); ctx.fillStyle=grad; ctx.fillRect(-stickLength,-4,stickLength,8);
        ctx.fillStyle="#2196f3"; ctx.beginPath(); ctx.arc(0,0,6,0,Math.PI*2); ctx.fill(); ctx.restore();
    }

    function updateAvatar(player){
        const nameInput=document.getElementById(player+"Name").value.trim();
        const avatar=document.getElementById(player+"Avatar");
        if(nameInput.length===0){ avatar.textContent=player.toUpperCase(); } 
        else { avatar.textContent=nameInput.split(" ").map(w=>w[0]).join("").substring(0,2).toUpperCase(); }
    }

    function updateBallCount() {
        [1, 2].forEach(player => {
            const color = ownership[player];
            if (!color) return;

            const countLeft = balls.filter(b => !b.inPocket && b.color === color).length;
            document.getElementById(`p${player}BallCount`).textContent = countLeft;

            const container = document.getElementById(`p${player}Balls`);
            container.innerHTML = "";
            for (let i = 0; i < countLeft; i++) {
                const dot = document.createElement("div");
                dot.className = `ball-dot ${color}`;
                container.appendChild(dot);
            }
        });

        if (!ownership[1] && !ownership[2]) {
            const redCount = balls.filter(b => !b.inPocket && b.color === "red").length;
            const yellowCount = balls.filter(b => !b.inPocket && b.color === "yellow").length;

            document.getElementById("p1BallCount").textContent = redCount;
            document.getElementById("p2BallCount").textContent = yellowCount;

            const p1Container = document.getElementById("p1Balls");
            const p2Container = document.getElementById("p2Balls");
            p1Container.innerHTML = ""; for(let i=0;i<redCount;i++){ const dot=document.createElement("div"); dot.className="ball-dot red"; p1Container.appendChild(dot);}
            p2Container.innerHTML = ""; for(let i=0;i<yellowCount;i++){ const dot=document.createElement("div"); dot.className="ball-dot yellow"; p2Container.appendChild(dot);}
        }
    }

    function endGame(winner, loser, foul = false) {
        let title = "Game Over";
        let message = "";
        if (foul) { message = `⚠️ Foul! ${loser} potted the 8-ball early.<br><strong>${winner} Wins!</strong>`; }
        else { message = `<strong>${winner} Wins!</strong> 🎉`; }
    ...
    window.restartGame = function () {
        ...
        blackPottedThisShot = false;
        firstContact = null;
        updateBallCount();
        document.getElementById("gameMessage").innerHTML = "";
        document.getElementById("turnIndicator").innerText = "Turn: Player 1";
        rackBalls();
        drawTable(); for (const ball of balls) { ball.draw(ctx); }
    };

    window.exitGame = function () { window.location.href = "/dashboard"; };

    function update(){ if(!gameOver){ let moving=false; for(let b of balls){ b.update(); if(b.speed()>0) moving=true; } for(let i=0;i<balls.length;i++){ for(let j=i+1;j<balls.length;j++){ resolveCollision(balls[i],balls[j]); } } if(ballsMoving&&!moving){ ballsMoving=false; endOfShot(); } } }
    function render(){ drawTable(); for(let b of balls)b.draw(ctx); drawAim(); drawCue(); document.getElementById("turnIndicator").textContent="Turn: "+getPlayerName(currentPlayer); }
